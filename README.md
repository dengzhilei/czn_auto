...
**v0.1.15 已更新支持国服简中版本。** 启动时可以选择 `自动`、`简体`、`繁体`，不确定就选 `自动`。

普通用户不需要安装 Python，也不用会写代码。下载发布版安装包，安装后从桌面图标或开始菜单正常启动即可。

## 这版更新

- 重点更新：已支持 `<国服简中>` 版本，内置简体识别图片，并保留繁体/默认模板。
- 默认桌面入口启动时会弹出语言选择：`自动`、`简体`、`繁体`。
...
- 修复部分三选一画面识别分数偏低时不继续行动、一直停在卡片页的问题。
...
2. 下载 `CZNAutoSetup-0.1.15.exe`。

3. 双击安装，安装时保留桌面快捷方式。

4. 打开游戏，切到需要开始自动流程的界面(进入卡厄思，选难度界面)。

5. 启动桌面上的 `CZN Auto`，弹出语言选择时选 `自动`。如果明确是国服简中，也可以选 `简体`。
...
1. 下载并运行 `CZNAutoSetup-0.1.15.exe`。
2. 安装时建议保留桌面快捷方式。
...
```text
templates\zh-Hans\   简体识别图
templates\           默认/繁体识别图
```

正常用户不用管这个。识别不稳时，可以用开始菜单里的简体/繁体入口，或在 `config.json` 里把 `input.ui_language` 改成 `zh-Hans` / `zh-Hant`。其中 `zh-Hant` 会优先找繁体覆盖目录；没有覆盖目录时就使用根目录模板。
...
dist\installer\CZNAutoSetup-0.1.15.exe
```
